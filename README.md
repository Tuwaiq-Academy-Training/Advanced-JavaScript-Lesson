...
* التعامع مع الـmethod في المصفوفات
* التعرف على الـفانكشن 
* التعرف على مفهوم الـDOM والتعامع مع الاحداث
* التعامل مع الـAPI
* التعرف على مفهوم الـPromises
...
قبل البدء في شرح مفهوم API بشكل تقني، سنقوم بشرح API بمثال من واقع الحياة لتقريب الفكرة، على سبيل المثال هدفنا من استخدام المصعد هو التنقل بين الأدوار دون الأخذ بالاعتبار كيف تتم هذه العملية، وكذلك عند تشغيل محرك السيارة فالهدف هو الانتقال للوجهة دون الحاجة لمعرفة خطوات عملية التشغيل.


وهذا ما يحدث مع APIs في الأكواد، نقوم باستخدام دالة معينة وننتظر منها تنفيذ العملية دون الاهتمام بالخطوات التي تقوم بها لإرجاع القيمة لك. ومن الأمثلة على استخدام API في لغة JavaScript هو استخدام المكتبات على سبيل المثال عند استخدام مكتبة Math ودالة sqrt الخاصة بإرجاع قيمة الجذر التربيعي لأي قيمة فنحن لا يهمنا كيفية تنفيذ هذه العملية ما يهمنا فقط هو النتيجة.
